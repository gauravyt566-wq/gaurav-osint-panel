...
const advancedFeatures = {
        ...
        if (item.isBulk) {
          ...
          return `
            <div class="history-item" onclick="advancedFeatures.reSearch('${item.type}', '${item.value}')">
              ...
              <div>
                <span class="history-type">${item.type.toUpperCase().replace('-', ' ')}</span>
  ...
  exportPDF() {
    ...
    } else if (lookupType === 'family-info') {
      reportTitle = 'FAMILY INFO ANALYSIS REPORT';
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('--- RECORD')) {
        ...
        yPosition += 10;
                 ...
                 line.includes('Mobile Number:') || line.includes('Aadhaar Number:') || line.includes('State:') ||
                 line.includes('District:') || line.includes('Scheme:') || line.includes('Ration Card ID:') ||
                 line.includes('Relationship:') || line.includes('Aadhaar Linked:')) {
        ...
        if (parts.length >= 2) {
          ...
          doc.text(label + ':', 15, yPosition); // Fixed: Equal left margin
          doc.setFont('helvetica', 'normal');
          doc.setTextColor(0, 0, 0);
...
const familyInfoApi = "https://family-members-n5um.vercel.app/fetch?key=paidchx&aadhaar=";
...
function formatJSON(rawJson, type, inputValue) {
  ...
  switch (type) {
    ...
    case 'ifsc':
      ...
      break;
    case 'family-info':
      // Family info has a different structure
      records = [data]; // Treat the whole response as a single record
      break;
  }

  if (records.length === 0 || (type === 'family-info' && !data.memberDetailsList)) {
    const cleanResponse = cleanServerResponse(rawJson);
  ...
  records.forEach((item, idx) => {
    if (type === 'family-info') {
      // Custom formatting for Family Info API
      formatted += "-------------------------------------------\n";
      formatted += `            RATION CARD & FAMILY DETAILS\n`;
      formatted += "-------------------------------------------\n";
      if (item.address) formatted += "Address: " + item.address.toUpperCase() + "\n";
      if (item.homeStateName) formatted += "State: " + item.homeStateName.toUpperCase() + "\n";
      if (item.homeDistName) formatted += "District: " + item.homeDistName.toUpperCase() + "\n";
      if (item.schemeName) formatted += "Scheme: " + item.schemeName.toUpperCase() + " (ID: " + item.schemeId + ")\n";
      if (item.rcId) formatted += "Ration Card ID: " + item.rcId + "\n\n";

      formatted += "-------------------------------------------\n";
      formatted += "                  FAMILY MEMBERS\n";
      formatted += "-------------------------------------------\n";
      if (item.memberDetailsList && item.memberDetailsList.length > 0) {
        item.memberDetailsList.forEach((member, memberIdx) => {
          formatted += `\n[${memberIdx + 1}] Name: ${member.memberName.toUpperCase()}\n`;
...
async function search(){
  ...
  switch (type) {
    ...
    case 'aadhaar':
      ...
      break;
    case 'family-info':
      url = familyInfoApi + encodeURIComponent(val);
      break;
    case 'gst':
